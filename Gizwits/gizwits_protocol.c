...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagio)
    {
        info->event[info->num] = EVENT_io;
        info->num++;
        dataPoints->valueio = gizStandardDecompressionValue(io_BYTEOFFSET,io_BITOFFSET,io_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueio != cur->valueio)
    {
        GIZWITS_LOG("valueio Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(io_BYTEOFFSET,io_BITOFFSET,io_LEN,(uint8_t *)devStatusPtr,dataPoints->valueio);
